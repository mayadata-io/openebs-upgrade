...
// UpdateFn is a typed function that abstracts an update
// against an unstructured instance
type UpdateFn func(*unstructured.Unstructured) (*unstructured.Unstructured, error)
...
// ForEachUpdate loops through this list and runs each item
// against the provided update function.
//
// Note: This function should be used when it is required to update
// the items of a list.
func (i *SliceIteration) ForEachUpdate(update OperationFn) error {
	for idx, item := range i.Items {
		// we must convert this item to an unstructured instance
		unItem := i.ItemToUnstruct(idx, item)
		// execute this item against the given update function
		err := update(unItem)
		if err != nil {
			return err
		}
		i.Items[idx] = unItem.UnstructuredContent()["spec"]
	}
	return nil
}
